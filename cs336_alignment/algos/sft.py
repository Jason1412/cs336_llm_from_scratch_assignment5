...
from cs336_alignment.algos.sft_utils import log_generation

from transformers import AutoTokenizer, PreTrainedModel
import os
from vllm import SamplingParams
from cs336_alignment.algos.grpo_utils import REWARD_FN_MAP
...
from cs336_alignment.utils import clear_memory
import wandb
...
def sft_microbatch_train_step(
    ...
    Returns:
        tuple[torch.Tensor, dict[str, torch.Tensor]]:
            torch.Tensor of shape (1,): the SFT loss.
            dict[str, torch.Tensor]: dictionary with keys "policy_loss" and "response_mask_sum".
    """
    ...
    loss_scaled = loss_mean / gradient_accumulation_steps

    loss_scaled.backward()

    metadata = {
        "loss_before_scaling": loss.detach(),
    }
...
class SFTDataset(Dataset):
    ...
    def load_from_disk(cls, path: str, prompt_template_path: str):
        prompts, cots, answers = load_dataset(
            path, prompt_template_path=prompt_template_path
        )
...
def sft_collate_fn(batch, tokenizer):
    """
    Args:
        batch: list[tuple[str, str, str]] List of (prompt, cot, answer) tuples.
        tokenizer: PreTrainedTokenizer from transformers.
    Returns:
        dict[str, torch.Tensor] Dictionary with keys "input_ids", "labels", "response_mask"
    """
...
class SFTTrainer:
    def __init__(
        ...
        train_config.to_json(os.path.join(self.checkpoint_path, "train_config.json"))
        ...
        self.cur_step = 0
        self.ctx = get_ctx(
            use_mixed=self.train_config.mixed_precision_training, device=device
        )
    ...
    def load_from_checkpoint(
        cls, model, checkpoint_path: str, device: torch.device
    ) -> "SFTTrainer":
        state = torch.load(os.path.join(checkpoint_path), map_location=device)
        train_config = SFTTrainingConfig.from_json(
            os.path.join(checkpoint_path, "train_config.json")
        )
        model.load_state_dict(state["model_state_dict"])
        trainer = cls(model=model, train_config=train_config, device=device)
    ...
    def evaluate(self, vllm=None):
        print_color(
            f"Evaluating SFT model on test dataset at step {self.cur_step}",
            color="magenta",
        )
    ...
    @torch.no_grad()
    def sample_responses(self, vllm=None, num_samples: int = 5):

        print_color(f"Sampling {num_samples} response from SFT model...", color="cyan")

        indices = random.sample(range(len(self.test_prompts)), k=num_samples)
        prompts = [self.test_prompts[i] for i in indices]
        answers = [self.test_true_answers[i] for i in indices]
    ...
    def train_step(
        ...
        for _ in trange(
            ...
            response_mask = micro_batch["response_mask"].to(
    ...
    def train(self, vllm=None):
        ...
        print_color(
            "||Training on dataset: " + self.train_config.dataset_name, color="green"
        )
        print_color(
